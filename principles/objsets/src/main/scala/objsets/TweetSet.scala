...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    def moreRetweeted(t1: Tweet, t2: Tweet) =
...
object GoogleVsApple {
  ...
  def filterByList(input: List[String]):TweetSet = {
    TweetReader.allTweets.filter(tweet => input.exists(keyword => tweet.text.contains(keyword)))
  }
