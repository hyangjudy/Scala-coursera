...
object Anagrams extends AnagramsInterface {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x::xs =>
      for {
        xSubTimes <- (0 to x._2).toList
        xsSub <- combinations(xs)
      } yield if(xSubTimes==0) xsSub else (x._1, xSubTimes)::xsSub.distinct
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.foldLeft(Map[Char, Int]())((acc, yi) => acc ++ Map(yi._1 -> yi._2))
    // xMap
    x.foldLeft(Map[Char, Int]())((acc, xi) => acc ++ Map(xi._1 -> (xi._2 - yMap.getOrElse(xi._1, 0))))
      .filter(xi => xi._2 > 0)
      .toList
    ...
    def iterate(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ => {
        for {
          ...
          if(dictionaryByOccurrences.contains(combinations))
